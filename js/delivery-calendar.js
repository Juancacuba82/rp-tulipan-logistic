            ...
            try {
                ...
                renderedTrips.forEach((rowData, idx) => {
                    try {
                        ...
                        // --- NEW: Context Menu (Right Click) for Quick Copy ---
                        tr.addEventListener('contextmenu', (e) => {
                            e.preventDefault();
                            showTripsContextMenu(e, rowData);
                        });
...
window.showTripsContextMenu = function(e, rowData) {
    let menu = document.getElementById('trips-context-menu');
    if (!menu) {
        menu = document.createElement('div');
        menu.id = 'trips-context-menu';
        menu.style.cssText = `
            display: none; position: fixed; z-index: 10000;
            background: white; border-radius: 8px;
            box-shadow: 0 4px 15px rgba(0,0,0,0.2);
            padding: 8px 0; min-width: 250px; max-height: 400px; overflow-y: auto;
    ...
    // Get the row element from the event target
    const tr = e.target.closest('tr');
    if (!tr || !tr.children) return;

    // Helper to safely get text from a cell
    const getCellText = (index) => {
        if (index >= tr.children.length) return '';
        const text = tr.children[index].textContent || tr.children[index].innerText || '';
        return text.trim();
    };

    menu.innerHTML = ''; // Clear previous

    const addMenuItem = (label, textToCopy, icon) => {
        if (!textToCopy || textToCopy === '---' || textToCopy === 'TBA' || textToCopy === '') return;
        const item = document.createElement('div');
        item.style.cssText = `
            padding: 10px 16px; cursor: pointer; color: #1e293b; font-size: 14px;
            display: flex; align-items: center; transition: background 0.2s;
            border-bottom: 1px solid #f1f5f9;
        `;
        
        // Truncate text for display if it's too long
        let displayText = textToCopy;
        if (displayText.length > 20) displayText = displayText.substring(0, 20) + '...';

        item.innerHTML = `<i class="${icon}" style="margin-right: 10px; color: #3b82f6; width: 16px; text-align: center;"></i> Copy ${label}: <strong style="margin-left: 5px;" title="${textToCopy}">${displayText}</strong>`;
        ...
        item.onclick = async () => {
            try {
                ...
                toast.style.cssText = `
                    ...
                    font-size: 14px; z-index: 10001; font-weight: bold; box-shadow: 0 4px 6px rgba(0,0,0,0.1);
    ...
    // Map visible table columns (0-indexed based on UI)
    addMenuItem('N. Cont', getCellText(2), 'fas fa-box');
    addMenuItem('Booking #', getCellText(3), 'fas fa-book');
    addMenuItem('Release #', getCellText(4), 'fas fa-file-alt');
    addMenuItem('Order #', getCellText(5), 'fas fa-hashtag');
    addMenuItem('City', getCellText(6), 'fas fa-city');
    addMenuItem('Pick Up', getCellText(7), 'fas fa-arrow-up');
    addMenuItem('Delivery', getCellText(8), 'fas fa-arrow-down');
    addMenuItem('Customer', getCellText(11), 'fas fa-user-tie');
    
    // Also provide a "Copy Row Summary" option
    const rowSummary = `${getCellText(2)} / ${getCellText(3)} / ${getCellText(5)}`;
    addMenuItem('Summary (Cont/Book/Ord)', rowSummary, 'fas fa-copy');

    if (menu.children.length === 0) {
        menu.innerHTML = '<div style="padding: 10px 16px; color: #94a3b8; font-size: 14px; font-style: italic;">Nothing to copy here</div>';
    } else {
        // Remove border from last item
        menu.lastChild.style.borderBottom = 'none';
    }

    // Append CSS for animation if not exists
    ...
    // Position logic to prevent clipping off-screen
    let x = e.clientX;
    let y = e.clientY;
    
    // Get actual menu dimensions (need to briefly display it to measure)
